...
// CmdChange is next change log tool
var CmdChange = &cobra.Command{
	Use:   "changelog",
	Short: "Get a next change log",
	Long:  "Get a next release or commits info. Example: next changelog dev or mext changelog {version}",
	Run:   run,
}
...
func init() {
	if repoURL = os.Getenv("KRATOS_REPO"); repoURL == "" {
		repoURL = "https://github.com/nextmicro/next.git"
	}
